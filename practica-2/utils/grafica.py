...
def dibuPtosRecta(entradas, salida, W, b, titulos=[], ph=0):
    if (entradas.shape[1] == 2):
       # --- DIBUJA LOS EJEMPLOS EN EL FONDO -----
        ...
        plt.axis([min(entradas[:, 0])-0.05, max(entradas[:, 0])+0.05,
                 min(entradas[:, 1])-0.05, max(entradas[:, 1])+0.05])
        plt.setp(plt.gca(), autoscale_on=False)

        clases = np.unique(salida)
        if len(clases) == 2:
            plt.plot(entradas[salida == min(clases), 0],
                     entradas[salida == min(clases), 1], 'bo')
            plt.plot(entradas[salida == max(clases), 0],
                     entradas[salida == max(clases), 1], 'ro')
        else:
